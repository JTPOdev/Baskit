...
{   
    // --------- ADD TO CART/BASKIT -------- //
    public static function addToCart($userId, $productId, $price, $productFee, $tagabiliFee, $quantity, $portion, $conn, $productImageUrl)
    {
        ...
        if ($result->num_rows > 0) {
            $sql = "UPDATE cart 
                    SET product_quantity = product_quantity + ?, product_price = ?, product_fee = ?, tagabili_fee = ?, product_image = ? 
                    WHERE user_id = ? AND product_id = ? AND product_portion = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("idddsiis", $quantity, $price, $productFee, $productImageUrl, $userId, $productId, $portion);
        } else {
            $sql = "INSERT INTO cart 
                      (user_id, product_id, product_name, product_price, product_fee, tagabili_fee, product_quantity, product_portion, product_origin, store_id, store_name, product_image) 
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
