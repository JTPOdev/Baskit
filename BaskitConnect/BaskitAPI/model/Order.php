...
class Order {
    ...
    public static function completeOrderByCode($orderCode, $conn) {
        ...
        try {
            $sql = "UPDATE orders SET status = 'Completed', is_ready = 'Completed' WHERE order_code = ?";
            $stmt = $conn->prepare($sql);
            if (!$stmt) {
            ...
            $sql = "UPDATE cart SET  is_ready = 'Completed' WHERE user_id = ?";
            $stmt = $conn->prepare($sql);
            if (!$stmt) {
                throw new Exception('SQL Error: ' . $conn->error);
            }
            ...
            $conn->commit();
            return ['message' => 'Order and cart marked as completed'];
    
        } catch (Exception $e) {
            $conn->rollback();
            return ['message' => 'Transaction failed', 'error' => $e->getMessage()];
        }
    }
    ...
    public static function markOrderAsDone($orderCode, $conn) {
        $conn->begin_transaction();
    
        try {
            // Fetch all orders with the given order_code
            ...
            // Insert each order item into history
            $sql = "INSERT INTO history (user_id, product_id, product_name, product_price, product_fee, 
                                         product_quantity, product_portion, product_origin, store_id, store_name, 
                                         status, tagabili_fee, tagabili_firstname, tagabili_lastname, 
                                         tagabili_mobile, tagabili_email, order_code) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            foreach ($orders as $order) {
                $stmt->bind_param("iisdisssissdsssss", 
                    $order['user_id'], $order['product_id'], $order['product_name'], $order['product_price'], 
                    $order['product_fee'], $order['product_quantity'], $order['product_portion'], $order['product_origin'], 
                    $order['store_id'], $order['store_name'], $order['status'], $order['tagabili_fee'], 
                    $order['tagabili_firstname'], $order['tagabili_lastname'], $order['tagabili_mobile'], 
                    $order['tagabili_email'], $order['order_code']
                );
                $stmt->execute();
            }
            $stmt->close();
    
            $sql = "UPDATE orders SET is_ready = 'Done' WHERE order_code = ?";
            $stmt = $conn->prepare($sql);
            if (!$stmt) {
                throw new Exception('SQL Error: ' . $conn->error);
            }
            ...
            $sql = "UPDATE cart SET is_ready = 'Done' WHERE user_id = ?";
            $stmt = $conn->prepare($sql);
            if (!$stmt) {
                throw new Exception('SQL Error: ' . $conn->error);
            }
            $stmt->bind_param("i", $orders[0]['user_id']);
            $stmt->execute();
            $stmt->close();
            ...
            $stmt->bind_param("i", $orders[0]['user_id']); // Use user_id from the first order
            $stmt->execute();
            $stmt->close();
    
            // Commit transaction
            $conn->commit();
            return ['message' => 'All orders moved to history and removed from cart/orders'];
