...
class ProductController
{
    public static function create($data, $conn)
    {
        ...
        $is_1pc = isset($data['is_1pc']) ? (int)$data['is_1pc'] : 0;
        $is_1_4kg = isset($data['is_1_4kg']) ? (int)$data['is_1_4kg'] : 0;
        $is_1_2kg = isset($data['is_1_2kg']) ? (int)$data['is_1_2kg'] : 0;
        $is_1kg = isset($data['is_1kg']) ? (int)$data['is_1kg'] : 0;
    
        if (!$is_1pc && !$is_1_4kg && !$is_1_2kg && !$is_1kg) {
            http_response_code(400);
            echo json_encode(['message' => 'At least one unit type must be selected']);
            return;
        }
        ...
        $success = Product::createProduct(
            $conn, $name, $price, $category, $storeId, $productImgPath['success'],
    ...
    public static function deleteProductById($productId, $conn) {
        if (!isset($productId) || empty($productId)) {
            header('HTTP/1.1 400 Bad Request');
            return ['message' => 'Product ID is required'];
        }
    
        $deleted = Product::deleteProduct($conn, $productId);
