...
class LoginController(private val lifecycleOwner: LifecycleOwner, private val context: Context) {
    ...
    fun login(
        email: String,
        password: String,
        onResult: (Boolean, String, String?, Map<String, String>) -> Unit // Includes errors map
    ) {
        lifecycleOwner.lifecycleScope.launch {
            ...
            try {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    // Successfully logged in, return success result
                    onResult(true, loginResponse?.message ?: "Login Successful!", role, emptyMap())
                } else {
                    // If response is not successful, parse the errors from the response
