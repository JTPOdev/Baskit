...
class UserStoreController(private val lifecycleOwner: LifecycleOwner, private val context: Context) {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    if (storeList != null && storeList.isNotEmpty()) {
                        val store = storeList.first() // Get the first store if it's not empty
    ...
    // Save store data to shared preferences for local access
    ...
    // Retrieve store details from shared preferences
